...
import os
os.chdir(r'I:\Quant\O3Quant\Models\DoubleCarry')
import pdblp as bbg
...
# Dados da BBG.
conn = bbg.BCon(debug=False, port=8194, timeout=10000)
conn.start()

px = conn.bdh(list(dic_tickers['ticker']), ["PX_LAST"], start, end)
...
#data_complete.at[data_ajuste, a] = row_list
data_complete.loc[data_ajuste] = row_list
...
# Ajuste da notação das moedas.
positions_final['weight'] = np.where(positions_final['ticker'].isin(inverter),
                                    -positions_final['weight'],
                                    positions_final['weight'])

data_path = 'I:/Quant/Data/'

positions_final.to_csv(data_path + 'positions_dc.csv', decimal=',', index = False)
resumo.to_csv(data_path + 'resumo_dc.csv',decimal =',', index = False)
port_total.to_csv(data_path + 'returns_dc.csv',decimal =',')
